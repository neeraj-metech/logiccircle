...
class PdfMaker extends Controller
{
	...
	function downloadpdf(Request $request){
		...
		$limitData = Setlimit::first()->toArray();
		$limit = $limitData['set_limit'];
		if(!empty($user)){
			$user = $user->toArray();
			...
			if($limit > $download_count){
				...
				$templateData = array(
                    ...
                    "url" => url('/login'),
                    'pdf_link'=> url('/downloadpdf').'/'.encrypt($email)
                );
				$html = $this->templateHtml($templateData);
				$pdf = App::make('dompdf.wrapper');
				$pdf->loadHTML($html);
				return $pdf->download('template.pdf');
			}else{
				return '<h2 style="text-aling:center;">Downoaded limit exist.</h2>';
			}
		}
		abort(404);
	}

	public function templateHtml($data){
		$html = '<!doctype html>
											...
											<tr>
												<td align="left" style="font-size:0px;padding:10px 25px;word-break:break-word;">
													...
													<div style="font-family:Helvetica Neue,Arial,sans-serif;font-size:16px;line-height:22px;text-align:left;color:#555;">
														...
														Thank you for signing up for '.$data['product'].' We\'re really happy to have you! Click the link below to login to your account:
													</div>
														...
														<tr>
															<td align="center" bgcolor="#2F67F6" role="presentation" style="border:none;border-radius:3px;color:#ffffff;cursor:auto;padding:15px 25px;" valign="middle">
																<p style="background:#2F67F6;color:#ffffff;font-family:Helvetica Neue,Arial,sans-serif;font-size:15px;font-weight:normal;line-height:120%;Margin:0;text-decoration:none;text-transform:none;">
																	<a href="'.$data['url'].'" >Login to Your Account
																</p>
															</td>
														</tr>
													</table>
